...
private:
	...
	/* If true the menu entries will be horizontally adjacent instead of vertically */
	bool horizontal;
...
public:
	 ...
	 * -1 if hte mouse is outside the gui */
	int getEntry(const sf::Vector2f mousePos);

	/* Change the text of an entry */
	void setEntry(int entry, std::string text);
	...
	void hightlight(const int entry);

	/* Return theee message bound to the entry*/
