...
public:
	...
	/* Check if one position in the map is connected to another by only traversing tiles in the whitelist */
	void findConnectedRegions(std::vector<TileType> whitelist, int type);

	/* Update the direction of the directional tiles so that they face the correct way.  Used to orient roads, pylons, rivers, etc. */
